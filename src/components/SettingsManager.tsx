...
export function SettingsManager({ initialImage, initialLogo, initialCommentAvatar, initialNotes }: { initialImage: string; initialLogo: string; initialCommentAvatar: string; initialNotes: Note[] }) {
  ...
  const [uploading, setUploading] = useState<"banner" | "logo" | "comment" | "">("");

  async function saveSettings(body: { promotionImage?: string; logoUrl?: string; commentAvatarUrl?: string }, success: string) {
    setNotice("Saving…");
  ...
  async function saveCommentAvatar(next = commentAvatar) {
    await saveSettings({ commentAvatarUrl: next }, "Comment avatar saved. Guests without a profile photo will use this image.");
  }

  async function upload(kind: "banner" | "logo" | "comment", event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) return;
    setUploading(kind); setNotice("Uploading image…");
    ...
    if (response.ok && result.url) {
      if (kind === "banner") { setImage(result.url); await saveBanner(result.url); }
      else if (kind === "logo") { setLogo(result.url); await saveLogo(result.url); }
      else { setCommentAvatar(result.url); await saveCommentAvatar(result.url); }
  ...
  return <section className="admin-list-page settings-manager">
    <header className="admin-page-title"><div><p className="section-label">SITE SETTINGS</p><h1>Settings</h1><p>Manage your logo, comment avatars, article banner and personal notes.</p></div></header>
    ...
    <section className="settings-section settings-logo-section">
      <header className="settings-section-heading"><div><p className="section-label">BRANDING</p><h2>Site logo</h2><p>Upload an image to replace the logo in the header, footer and admin.</p></div></header>
      <form onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); void saveLogo(); }}>
        <div className="settings-logo-preview"><img src={logo} alt="Site logo preview" /></div>
        <label>Logo image URL<input value={logo} onChange={(event) => setLogo(event.target.value)} required /></label>
        <label className="settings-upload">Upload a logo image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={(event) => void upload("logo", event)} disabled={Boolean(uploading)} /><span>{uploading === "logo" ? "Uploading…" : "Choose image"}</span></label>
        <footer>
          <small>{notice}</small>
          <div className="settings-logo-actions">
            <button type="button" className="settings-reset" disabled={Boolean(uploading)} onClick={() => { setLogo(defaultLogoUrl); void saveLogo(defaultLogoUrl); }}>Use default</button>
    ...
    <section className="settings-section settings-comment-avatar-section">
      <header className="settings-section-heading"><div><p className="section-label">COMMENTS</p><h2>Default comment avatar</h2><p>This image is shown for readers who have not uploaded a profile photo.</p></div></header>
      <form onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); void saveCommentAvatar(); }}>
        <div className="settings-comment-avatar-preview"><img src={commentAvatar} alt="Default comment avatar preview" /></div>
        <label>Comment avatar URL<input value={commentAvatar} onChange={(event) => setCommentAvatar(event.target.value)} required /></label>
        <label className="settings-upload">Upload a comment avatar<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={(event) => void upload("comment", event)} disabled={Boolean(uploading)} /><span>{uploading === "comment" ? "Uploading…" : "Choose image"}</span></label>
        <footer>
          <small>{notice}</small>
          <div className="settings-logo-actions">
            <button type="button" className="settings-reset" disabled={Boolean(uploading)} onClick={() => { setCommentAvatar(defaultCommentAvatarUrl); void saveCommentAvatar(defaultCommentAvatarUrl); }}>Use default</button>
            <button disabled={Boolean(uploading)}>Save avatar</button>
          </div>
        </footer>
      </form>
    </section>
    <section className="settings-section settings-banner-section"><header className="settings-section-heading"><div><p className="section-label">ARTICLE PROMOTION</p><h2>Promotional banner</h2><p>Choose the image shown below article tables of contents.</p></div></header><form onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); void saveBanner(); }}><div className="settings-banner-preview"><img src={image} alt="Article promotional banner preview" /></div><label>Banner image URL<input type="url" value={image} onChange={(event) => setImage(event.target.value)} required /></label><label className="settings-upload">Upload a new banner image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={(event) => void upload("banner", event)} disabled={Boolean(uploading)} /><span>{uploading === "banner" ? "Uploading…" : "Choose image"}</span></label><footer><small>{notice}</small><button disabled={Boolean(uploading)}>Save banner</button></footer></form></section>
  </section>;
}
