...
import { adminWriteHeaders, currentAdmin, parseConfigured } from "@/lib/admin";
...
export async function POST(request: Request) {
  ...
  const { url } = await import("@/lib/admin");
  const safeName = image.name.replace(/[^a-zA-Z0-9._-]/g, "-").slice(-100) || "profile-image";
  if (r2Configured) {
    try {
      const url = await uploadToR2(`avatars/${crypto.randomUUID()}-${safeName}`, await image.arrayBuffer(), image.type);
  ...
  const response = await fetch(`${url}/files/${encodeURIComponent(`${crypto.randomUUID()}-${safeName}`)}`, { method: "POST", headers: { ...adminWriteHeaders, "Content-Type": image.type }, body: image });
