...
export async function PUT(request: Request) {
  ...
  const body = await request.json() as { promotionImage?: unknown; logoUrl?: unknown; commentAvatarUrl?: unknown };
  const payload: { key: string; promotionImage?: string; logoUrl?: string; commentAvatarUrl?: string } = { key: "primary" };
  ...
  if (!payload.promotionImage && !payload.logoUrl && !payload.commentAvatarUrl) return NextResponse.json({ error: "Nothing to save." }, { status: 400 });
  ...
  const response = await fetch(id ? `${url}/classes/SiteProfile/${encodeURIComponent(id)}` : `${url}/classes/SiteProfile`, { method: id ? "PUT" : "POST", headers: adminWriteHeaders, body: JSON.stringify(payload) });
  if (!response.ok) return NextResponse.json({ error: "Settings could not be saved." }, { status: 500 });
