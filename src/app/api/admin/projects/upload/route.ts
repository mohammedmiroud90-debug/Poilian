...
export async function POST(request: Request) {
  ...
  if (!(image instanceof File) || image.size > 3 * 1024 * 1024) {
    return NextResponse.json({ error: "Upload a PNG, JPG, WebP, or GIF image smaller than 3 MB." }, { status: 400 });
  }
  ...
  if (r2Configured) {
    try {
      return NextResponse.json({
        url: await uploadToR2(`projects/${crypto.randomUUID()}-${name}`, await image.arrayBuffer(), image.type),
        storage: "r2",
      });
    } catch {
