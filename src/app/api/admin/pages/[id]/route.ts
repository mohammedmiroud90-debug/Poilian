...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const response = await fetch(`${url}/classes/SitePage/${encodeURIComponent(id)}`, { method: "DELETE", headers });
  return response.ok ? NextResponse.json({ ok: true }) : NextResponse.json({ error: "Page could not be deleted." }, { status: 500 });
}
